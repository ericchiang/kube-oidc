...
func (c *claimsTest) run(t *testing.T) {
	...
	gotUsername, gotGroups, ok, err := a.AuthenticateToken(token)
	if err != nil {
		if !c.wantErr {
	...
	if gotUsername != c.wantUsername {
		t.Errorf("expected username=%v, got=%v", c.wantUsername, gotUsername)
	}
	if len(gotGroups) == 0 && len(c.wantGroups) == 0 {
		return
	}
	if !reflect.DeepEqual(gotGroups, c.wantGroups) {
		t.Fatalf("expected groups=%#v, got=%#v", c.wantGroups, gotGroups)
	}
}
...
func TestToken(t *testing.T) {
	...
	tests := []claimsTest{
		...
		},
		{
			...
			wantUsername: "oidc:jane",
			wantGroups:   []string{"groups:team1", "groups:team2"},
		},
		{
