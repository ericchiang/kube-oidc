...
/*
Package oidc is a fork of the Kubernetes OpenID Connect authenticator.
...
// after creation.
func newAsyncIDTokenVerifier(ctx context.Context, logger *log.Logger, c *oidc.Config, iss string) *asyncIDTokenVerifier {
	...
	initFn := func() (done bool, err error) {
		...
		if err != nil {
			logf(logger, "oidc authenticator: async token verifier for issuer: %q: %v", iss, err)
			return false, nil
		}
		t.m.Lock()
		defer t.m.Unlock()
	...
	go func() {
		if done, _ := initFn(); !done {
			go pollUntil(time.Second*10, initFn, ctx.Done())
		}
	}()
...
// Authenticator evaluates OpenID Connect ID Tokens and maps them to a username and group.
...
// Close brings down any goroutines associated with the Authenticator.
func (a *Authenticator) Close() {
	a.cancel()
}

// New initializes an authenticator with the given options.
func New(opts Options) (*Authenticator, error) {
	return newAuthenticator(opts, func(ctx context.Context, a *Authenticator, config *oidc.Config) {
		...
		go pollUntil(time.Second*10, func() (done bool, err error) {
			provider, err := oidc.NewProvider(ctx, a.issuerURL)
			if err != nil {
				logf(opts.Logger, "oidc authenticator: initializing plugin: %v", err)
				return false, nil
			}
...
func newAuthenticator(opts Options, initVerifier func(ctx context.Context, a *Authenticator, config *oidc.Config)) (*Authenticator, error) {
	...
	if opts.CAFile != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("oidc: failed to read CA file: %v", err)
		}
		roots = x509.NewCertPool()
		if !roots.AppendCertsFromPEM(data) {
			return nil, fmt.Errorf("oidc: CA file didn't contain any PEM encoded certificates")
		}
	}
...
func (r *claimResolver) Verifier(iss string) (*oidc.IDTokenVerifier, error) {
	...
	if av == nil {
		...
		av = newAsyncIDTokenVerifier(ctx, r.logger, r.config, iss)
...
// AuthenticateToken authenticates an OpenID Connect ID Token.
func (a *Authenticator) AuthenticateToken(token string) (username string, groups []string, ok bool, err error) {
	...
	if a.groupsPrefix != "" {
		for i, group := range userGroups {
			userGroups[i] = a.groupsPrefix + group
		}
	}
	...
	return username, userGroups, true, nil
}
